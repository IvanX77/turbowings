...
// The time at which LionWings was booted. No JWT's created before this time are allowed to
// connect to the socket since they may have been marked as denied already and therefore
// could be invalid at this point.
//
// By doing this we make it so that a user who gets disconnected from LionWings due to a LionWings
...
// before LionWings was booted, or because we have denied all tokens with the same JTI
...
func (p *WebsocketPayload) Denylisted() bool {
	...
	// If the time that the token was issued is before the time at which LionWings was booted
